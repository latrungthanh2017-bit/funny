...
# ==========================================
# TÍCH HỢP CORE BANKING (đã tách thành phân hệ riêng)
# ==========================================
elif menu.startswith("Tích hợp"):
    render_header("Tích hợp dữ liệu Core Banking & Chấm điểm tự động")
    st.write("Tải file dữ liệu lỗi trích xuất từ hệ thống Core Banking để hệ thống tự động chấm điểm và khuyến nghị theo bộ quy tắc.")
    ...
    if uploaded_file is not None:
        try:
            ...
            if "Mức_Độ_Ảnh_Hưởng" in df_core.columns:
                ...
                critical_rate = (critical_errors / total_records) * 100 if total_records else 0
            else:
            ...
            col_b.metric("Số lỗi trọng yếu (Mức 4 & 5)", f"{critical_errors} lỗi", f"Tỷ lệ: {critical_rate:.1f}%")

            st.markdown("---")
            st.markdown("### ⚙️ Khuyến nghị từ hệ thống quy tắc (Rule-based)")
            if total_records > 100:
                st.error(f"🚨 **Đề xuất 1 — Kích hoạt rà soát diện rộng.** Số lỗi trong tệp ({total_records}) vượt hạn mức an toàn. Khuyến nghị Kiểm soát Nội bộ kiểm tra đột xuất các chi nhánh có tần suất lỗi cao nhất.")
            else:
                st.success("✅ **Đề xuất 1:** Số lỗi vận hành trong tầm kiểm soát. Tiếp tục giám sát từ xa.")
            if critical_rate > 10:
                st.warning(f"⚠️ **Đề xuất 2 — Điều chỉnh trọng số KPI.** Tỷ lệ lỗi nghiêm trọng {critical_rate:.1f}% cho thấy dấu hiệu nới lỏng tuân thủ. Cần siết bộ lọc chặn giao dịch vượt thẩm quyền trên Core ngay.")
            else:
                st.info("💡 **Đề xuất 2:** Tỷ lệ rủi ro nghiêm trọng thấp. Các chốt kiểm soát tự động đang hoạt động hiệu quả.")
        except Exception as e:
            st.error(f"❌ File không đúng định dạng hoặc lỗi mã hóa: {e}")
    else:
        st.info("Gợi ý: file CSV nên có tối thiểu các cột **Mã_Lỗi, Chi_Nhánh, Mức_Độ_Ảnh_Hưởng** (1–5).")
